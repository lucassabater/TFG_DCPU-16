...
bool window_init(MainWindow *win) {
    ...
    win->window = SDL_CreateWindow("DCPU-16 Studio", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                                   win->window_w, win->window_h, SDL_WINDOW_SHOWN | SDL_WINDOW_ALLOW_HIGHDPI);
    ...
    win->lem_texture = SDL_CreateTexture(win->renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING, LEM1802_WIDTH, LEM1802_HEIGHT);
    ...
    if (nk_begin(win->ctx, "IDE", nk_rect(0, 0, win->window_w, win->window_h), NK_WINDOW_NO_SCROLLBAR | NK_WINDOW_BACKGROUND)) {
        ...
        if (nk_menu_begin_label(win->ctx, "File", NK_TEXT_CENTERED, nk_vec2(200, 200))) {
            ...
            if (nk_menu_item_label(win->ctx, "Open ROM...", NK_TEXT_LEFT)) {
                const char *filtros[2] = { "*.bin", "*.txt" };
                const char *archivo = tinyfd_openFileDialog("Selecciona una ROM", "", 2, filtros, "Archivos ROM", 0);
                if (archivo != NULL) {
                    emu_load_rom(emu, archivo);
                }
            }
            if (nk_menu_item_label(win->ctx, "Exit", NK_TEXT_LEFT)) {
                // Forzamos un evento de salida en SDL para cerrar limpiamente
        ...
        // --- PANTALLA CENTRAL ---
        nk_layout_row_dynamic(win->ctx, 20, 1); nk_spacing(win->ctx, 1);
